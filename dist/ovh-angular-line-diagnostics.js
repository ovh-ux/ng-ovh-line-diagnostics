...
angular.module("ovh-angular-line-diagnostics").controller("LineDiagnosticsCtrl", function () {
    LineDiagnosticsCtrl.$inject = ["$interval", "$timeout", "$translate", "$uibModal", "atInternet", "LineDiagnostics", "LineDiagnosticFactory", "Toast", "DIAGNOSTICS_CONSTANTS"];
    function LineDiagnosticsCtrl($interval, $timeout, $translate, $uibModal, atInternet, LineDiagnostics, LineDiagnosticFactory, Toast, DIAGNOSTICS_CONSTANTS) {
    ...
    _createClass(LineDiagnosticsCtrl, [{
        ...
        key: "confirmIntervention",
        value: function confirmIntervention() {
            var _this8 = this;

            this.openDialog("ovh-angular-line-diagnostics-confirm-intervention-dialog").then(function (dialogAnswer) {
                if (dialogAnswer) {
                    _this8.answerQuestion("conditionsAccepted", true);
                }
            });
        }
    }, {
